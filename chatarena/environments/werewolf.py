...
class Werewolf(Environment):
    ...
    def __init__(self, args, player_names: List[str], topic_codes: Dict[str, List[str]] = None, **kwargs):
        ...
        current_game_number = args.current_game_number

        wolves = ["werewolf", "werewolf", "werewolf"]
        leader = ["witch", "guard", "seer"]
        villager = ["villager", "villager"]

        experiment = 1

        if experiment ==0 and current_game_number % 2 == 1:
            origin = wolves + [villager[0]]
            agent = leader + [villager[0]]
        else:
            origin = leader + [villager[0]]
            agent = wolves + [villager[0]]

        random.shuffle(origin)

        # random.shuffle(agent)

        self._characters = origin + agent

        # self._characters = ["werewolf","werewolf","villager","seer","guard","werewolf","witch","villager"]
        ...
        for i, x in enumerate(self._characters):
            ...
            self.identity_record[x].append(i)
        # self.identity_record = self._identity_mapping
        ...
        self._day_votes = dict()
        self._night_votes = dict()
        self._lives = 0
    ...
    def _moderator_speak(self, text: str, visible_to: Union[str, List[str]] = "all", importance=1, msg_type=None):
        """
        ...
        if msg_type == None:
            ...
            message = Message(agent_name="Moderator", content=text, turn=self._current_turn, visible_to=visible_to,
                              importance=importance, msg_type=msg_type)
    ...
    def handle_vote(self, action, player_name):
        identity = self._characters[int(player_name.split()[1]) - 1]
        try:
        ...
        if self._current_phase == "daytime":
            ...
            if vote in self.player_names or vote == "pass":
                if self._current_turn in self._day_votes.keys():
                    self._day_votes[self._current_turn][identity][player_name] = vote
                else:
                    self._day_votes[self._current_turn] = {"werewolf": dict(), "witch": dict(), "guard": dict(),
                                                           "seer": dict(), "villager": dict()}
            ...
            # 处理晚上投票逻辑
            if vote in self.player_names or vote == "pass":
                if self._current_turn in self._night_votes.keys():
                    self._night_votes[self._current_turn][identity][player_name] = vote
                else:
                    self._night_votes[self._current_turn] = {"werewolf": dict(), "witch": dict(), "guard": dict(),
                                                             "seer": dict()}
                    self._night_votes[self._current_turn][identity][player_name] = vote

            # 其他代码

    def save_results(self, wins):
        identities = dict()
        for id in self.identity_record.keys():
            if id in ['werewolf', 'villager', 'seer', 'guard', 'witch']:
                identities[id] = self.identity_record[id]
        # identities = self.identity_record
        alive = self._identity_mapping

        wins = wins
        kill_list = self._killed_list
        agent = 0
        origin = 0
        ...
        data = {
            ...
            "win_agent": agent,
            "win_origin": origin,
            "day_votes": self._day_votes,
            "night_votes": self._night_votes

        }
        import os

        os.makedirs(f"./record2/test{self.args.version}", exist_ok=True)
        os.makedirs(f"./record2/test{self.args.version}/{self.args.current_game_number}", exist_ok=True)
        with open(
                f"./record2/test{self.args.version}/{self.args.current_game_number}/{self.args.current_game_number}.json",
                "w") as f:
            json.dump(data, f)
    ...
    def _check_game_over(self):
        ...
        if self._lives > 0 and len(self._identity_mapping["werewolf"]) == 0:
            self._moderator_speak(
                f"Game over, the villager {self.identity_record['villager']}、the witch {self.identity_record['witch']}、the guard {self.identity_record['guard']} and the seer{self.identity_record['seer']} wins!")
            _give_rewards(_get_winner_names(True), "villager")
            wins = self.identity_record['villager'] + self.identity_record['witch'] + self.identity_record['guard'] + \
                   self.identity_record['seer']
            self.save_results(wins)
            # with open(f"{self.args.current_game_number}.json","w") as f:
        ...
        # print(self.identity_record)
        if len(self._identity_mapping["werewolf"]) > 0 and (len(self._identity_mapping['villager']) + len(
                self._identity_mapping["guard"] + self._identity_mapping['witch'] + self._identity_mapping[
                    'seer']) == 0):
            self._moderator_speak(f"Game over, the werewolf {self.identity_record['werewolf']} wins!")
            _give_rewards(_get_winner_names(False), "werewolf")
            wins = self.identity_record['werewolf']
            self.save_results(wins)
            self._moderator_speak(SIGNAL_END_OF_CONVERSATION)
            return True
        return False
    ...
    def step(self, player_name: str, action: str) -> TimeStep:
        ...
        self.handle_vote(action, player_name)
        if self._current_phase == "daytime":
            assert self._get_number_of_people()[2] > 0
            rewards = self.get_zero_rewards()
            if self._number_of_rounds == 0:
                print(f"action: {action}", file=sys.stderr)
                message = Message(agent_name=player_name, content=action, turn=self._current_turn, msg_type="action")
                self.message_pool.append_message(message)
                ...
                if self._next_player_idx == self._current_first_alive:
                    ...
                    print(f"alive list: {self._alive_list}", file=sys.stderr)
                    self._moderator_speak(
                        f"Now you {self.get_next_player()} are asked to choose which of the players should be voted for killing based on the discussion? Don't mention your role. "
                ...
                if self._next_player_idx == self._current_first_alive:
                    ...
                    if to_kill == "pass":
                        ...
                        self._moderator_speak(f"Werewolves, please open your eyes! "
                                              ...
                                              # f"For example: I choose Player...",
                                              visible_to=[self.player_names[i] for i in
                                                          self._identity_mapping["werewolf"]],
                                              importance=1 if self._judge_is_alive(self.get_next_player()) else 0)
                    else:
                        print(f"to kill: {to_kill}", file=sys.stderr)
                        self._moderator_speak(f"{to_kill} will be killed! You can make a brief last statement.",
                                              importance=6)
                        self._next_player_idx = self.player_names.index(to_kill)
                        self._number_of_rounds = 9999
                else:
                    self._moderator_speak(
                        f"The next {self.get_next_player()}, you, continue voting the players should be killed based on the discussion? Don't mention your role. "
                ...
                if self._check_game_over():
                    return TimeStep(observation=self.get_observation(), reward=rewards,
                                    terminal=True), self.message_pool.current_messages
                self._current_turn += 1
                self._number_of_rounds = 0
                self._players_votes = {name: 0 for name in self.player_names}
                self._players_votes["pass"] = 0
                ...
                print(f"alive list: {self._alive_list}", file=sys.stderr)
                self._moderator_speak(f"It's dark, everyone close your eyes.")
                visible = [self.player_names[i] for i in self._identity_mapping["werewolf"]]

                self._moderator_speak(f"Werewolves: {','.join(visible)}, please open your eyes! "
                                      f"Now vote and tell your teammates which of the players should be killed tonight. "
                                      f"You {self.get_next_player()} only choose one from the following living options please: [{', '.join(self._alive_list)}]. ",
                                      # f"For example: I choose Player...",
                                      visible_to=visible,
            ...
            if self._next_player_idx < self.werewolf_numbers:
                ...
                message = Message(agent_name=player_name, content=action, turn=self._current_turn,
                                  visible_to=[self.player_names[i] for i in self._identity_mapping["werewolf"]],
                                  importance=1 if self._judge_is_alive(player_name) else 0, msg_type="action")
                self.message_pool.append_message(message)
                ...
                if self._next_player_idx == self.werewolf_numbers:
                    self._moderator_speak(f"You guard, {self.get_next_player()}, please open your eyes! "
                                          f"Now tell me who you protect tonight? (You can't protect the same one from the last night)."
                                          f"You only choose one from the following living options please: [{', '.join(self._alive_list)}]. ",
                    ...
                    self._moderator_speak(
                        f"The next werewolf, you {self.get_next_player()}, please vote and tell your teammates that which of the players should be killed tonight. "
                        f"You only choose one from the following living options please: [{', '.join(self._alive_list)}]. ",
                        # f"For example: I choose Player...",
                        visible_to=[self.player_names[i] for i in self._identity_mapping["werewolf"]],
                        importance=1 if self._judge_is_alive(self.get_next_player()) else 0)
                ...
                # 找到最大的票数
                max_votes = max(self._players_votes.values())
                # 找到所有具有最大票数的玩家
                potential_victims = [player for player, votes in self._players_votes.items() if votes == max_votes]
                # 随机选择一个玩家
                werewolf_kill = random.choice(potential_victims)
                self._moderator_speak(
                    f"You choiced to kill the {werewolf_kill}.",
                    visible_to=[self.player_names[i] for i in self._identity_mapping["werewolf"]],
                    importance=1 if self._judge_is_alive(self.get_next_player()) else 0)
                ...
                if len(self._killed_list) > 0 and self._witch_antidote:
                    self._moderator_speak(
                        f"You witch, {self.get_next_player()}, please open your eyes! {self._killed_list[0]} will be killed tonight. "
                        f"You have a bottle of antidote, do you want to save him? Note it! You only have one botle of antidote the whole game, you have to make carful considerations."
                        ...
                        importance=1 if self._judge_is_alive(self.get_next_player()) else 0, msg_type="important")
                else:
                    ...
                    self._moderator_speak(f"You witch, {self.get_next_player()}, please open your eyes! "
                                          f"You have a bottle of poison, who are you going to kill tonight? Note it! You only have one botle of poison the whole game, you have to make carful considerations."
                ...
                if self._number_of_rounds == 0:
                    if self._judge_is_alive(player_name):
                        print(f"action: {action}", file=sys.stderr)
                        if "yes" in action or "Yes" in action or "will use" in action or "choose to use" in action or "save" in action:
                    ...
                    self._print_infos()
                    self._moderator_speak(
                        f"You {self.get_next_player()} have a bottle of poison, who are you going to kill tonight? "
                ...
                if self._judge_is_alive(player_name):
                    ...
                    if self._characters[idx] == "werewolf":
                        self._moderator_speak(f"{self.player_names[idx]} is a werewolf!",
                                              visible_to=[self.player_names[i] for i in self._identity_mapping["seer"]],
                                              importance=5, msg_type="important")
                    else:
                ...
                if len(self._night_kill_list) != 0:
                    ...
                    if self._number_of_nights == 0:
                        killed_identity = [self._characters[self.player_names.index(name)] for name in
                                           self._night_kill_list]
                        print(f"killed_identity: {killed_identity}", file=sys.stderr)
                        self._moderator_speak(f"{', '.join(self._night_kill_list)} are {', '.join(killed_identity)}.",
                                              importance=5, msg_type="important")
                else:
                ...
                self._moderator_speak(
                    f"Now freely talk about roles of other players with each other based on your observation and "
        ...
        return timestep, self.message_pool.current_messages
