...
class Agent(Configurable):
    ...
    @abstractmethod
    def __init__(self, name: str, role_desc: str, global_prompt: str = None, graph = None, *args, **kwargs):
...
class Player(Agent):
    ...
    def __call__(self, args, observation: List[Message], messages: MessagePool, questions: QuestionPool, state = (0, "daytime", "", []),graph =None) -> str:
        """
        ...
        try:
            if self.name == "Player 1" and args and args.human_in_combat:
                ...
                response = self.backend.query(args, agent_name=self.name, role_desc=self.role_desc,
                                            ...
                                            day_night=state[1], role=state[2], alives=state[3],graph = graph)
