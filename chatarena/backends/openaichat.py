...
class OpenAIChat(IntelligenceBackend):
    ...
    def query(self, arg, agent_name: str, role_desc: str, history_messages: List[Message], msgs: MessagePool,
        ...
        response = self._get_response(request, conn_method, max_tokens=1500, T=temperature, *args, **kwargs)
        ...
        request = [system_prompt] + conversations + request_prompt
        print(f"request: {request}", file=sys.stderr)
        response = self._get_response(request, conn_method, max_tokens=1500, T=0.8, *args, **kwargs)
        ...
        with open(f"./record2/test{version}/{arg.current_game_number}/{arg.current_game_number}-{agent_name}.txt",
                  "w") as file:
