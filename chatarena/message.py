...
class MessagePool():
    ...
    def append_message(self, message: Message):
        ...
        if self.args and message.agent_name == "Moderator":
            with open(os.path.join(self.args.logs_path_to, str(self.args.current_game_number) + ".md"), "a") as f:
                ...
                f.write(output + "  \n")
            with open(os.path.join(self.args.logs_path_to, str(self.args.current_game_number) + "(2).md"), "a") as f:
                output = f"**{message.agent_name} (-> {str(message.visible_to)})**: {message.content}"
                f.write(output + "  \n")
    ...
    def get_visible_messages(self, agent_name, turn: int) -> List[Message]:
        ...
        prev_messages = [message for message in self._messages if message.turn <= turn and message.importance > 0 
                         and (message.msg_type == "text" or message.msg_type == "ref" or message.msg_type=="important" or message.msg_type=="inf" or message.msg_type=="action")]
...
class QuestionPool():
    ...
    def _initial_questions(self):
        return [
            ...
            Question(content="Is the seer alive? Which player may be the seer that is most threatening to us?", turn=0, visible_to="werewolf", reward=500),
            Question(content="Which player is another pretty girl(werewolf) in this game?", turn=0, visible_to="werewolf", reward=500),
            Question(content="Based on the conversation and my inference, who is most likely to be an alive pretty girl(werewolf) ?", turn=0, visible_to="villager", reward=500),
            Question(content="Which player made the statement claiming to be a seer? Can his words be trusted?", turn=0, visible_to="villager", reward=500),
            # Question(content="Are there any clues or information I can refer to for special characters such as seer, pharmacist and sentry?", turn=0, visible_to="villager", reward=500),
            ...
            Question(content="Is the prophet/seer still alive? If yes, who is the prophet/seer?", turn=0, visible_to="guard", reward=500),
            Question(content="Did the moderator tell me the identities of the other players(werewolves, non-wolves) last night?", turn=0,
                     visible_to="seer", reward=1000),
            Question(content="Who got killed by a werewolf last night? Did I use the antidote and the poison?",turn=0,visible_to="witch", reward=1000),
        ]
